...
function preparePeerConnection() {
    ...
    peerConnection.onconnectionstatechange = function() {
        console.log(`Chat WebRTC connection state: ${peerConnection.connectionState}`)
        if (peerConnection.connectionState === 'connected') {
            console.log('Chat WebRTC connection established - optimizing for low latency')
        }
    }
    
    // Optimize ICE gathering for faster connection
    peerConnection.onicegatheringstatechange = function() {
        console.log(`Chat ICE gathering state: ${peerConnection.iceGatheringState}`)
    }
    
    // Handle ICE connection state
    peerConnection.oniceconnectionstatechange = function() {
        console.log(`Chat ICE connection state: ${peerConnection.iceConnectionState}`)
    }
...
// Synchronize audio and video timing for chat interface
function syncAudioVideo() {
    const videoElement = document.querySelector('#remoteVideo video')
    const audioElement = document.querySelector('#remoteVideo audio')
    
    if (videoElement && audioElement) {
        // Ensure video and audio are at the same timestamp
        if (Math.abs(videoElement.currentTime - audioElement.currentTime) > 0.1) {
            console.log(`Chat A/V Sync: Video=${videoElement.currentTime.toFixed(2)}s, Audio=${audioElement.currentTime.toFixed(2)}s`)
            videoElement.currentTime = audioElement.currentTime
        }
    }
}

// Monitor latency and synchronization for chat
function monitorChatLatency() {
    const videoElement = document.querySelector('#remoteVideo video')
    const audioElement = document.querySelector('#remoteVideo audio')
    ...
    if (videoElement && audioElement) {
        ...
        if (timeDiff > 0.05) { // Log if difference is more than 50ms
            console.log(`Chat Latency: Video=${videoTime.toFixed(3)}s, Audio=${audioTime.toFixed(3)}s, Diff=${(timeDiff * 1000).toFixed(1)}ms`)
            
            // Auto-sync if difference is significant
            if (timeDiff > 0.1) {
                console.log('Auto-syncing chat audio-video timing')
                syncAudioVideo()
            }
        }
    }
...
// Start chat latency monitoring
function startChatLatencyMonitoring() {
    setInterval(monitorChatLatency, 100) // Check every 100ms
}
...
window.onload = () => {
    ...
    // Start chat latency monitoring after a short delay
    setTimeout(startChatLatencyMonitoring, 2000)
}
