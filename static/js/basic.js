...
function preparePeerConnection() {
    ...
    peerConnection.onconnectionstatechange = function() {
        ...
        if (peerConnection.connectionState === 'connected') {
            console.log('WebRTC connection established - optimizing for low latency')
        }
    }
    ...
    peerConnection.ontrack = function (event) {
        ...
        // Optimize video for low latency
        if (event.track.kind === 'video') {
            mediaPlayer.playsInline = true
            mediaPlayer.muted = true // Mute video to prevent audio conflicts
            mediaPlayer.preload = 'auto'
            
            // Set low-latency video properties
            mediaPlayer.style.transform = 'translateZ(0)' // Hardware acceleration
            mediaPlayer.style.willChange = 'transform' // Optimize for animations
            
            // Reduce video buffering for lower latency
            mediaPlayer.addEventListener('loadedmetadata', () => {
                // Set video properties for minimal latency
            ...
            mediaPlayer.addEventListener('play', () => {
                ...
                // Force video to render at optimal frame rate
                mediaPlayer.style.imageRendering = 'optimizeSpeed'
            })
            
            // Handle video timing for better sync
            mediaPlayer.addEventListener('timeupdate', () => {
                // Log video timing for debugging
                if (mediaPlayer.currentTime % 1 < 0.1) { // Log every second
                    console.log(`Video time: ${mediaPlayer.currentTime.toFixed(2)}s`)
                }
            })
        } else {
            // Audio track optimization
            mediaPlayer.muted = true // Start muted to prevent audio conflicts
            mediaPlayer.preload = 'auto'
            
            // Unmute audio when speaking starts
            mediaPlayer.addEventListener('canplay', () => {
...
function syncAudioVideo() {
    ...
    if (videoElement && audioElement) {
        // Ensure video and audio are at the same timestamp
        if (Math.abs(videoElement.currentTime - audioElement.currentTime) > 0.1) {
            console.log(`Syncing A/V: Video=${videoElement.currentTime.toFixed(2)}s, Audio=${audioElement.currentTime.toFixed(2)}s`)
...
function monitorLatency() {
    ...
    if (videoElement && audioElement) {
        ...
        if (timeDiff > 0.05) { // Log if difference is more than 50ms
            console.log(`Latency warning: Video=${videoTime.toFixed(3)}s, Audio=${audioTime.toFixed(3)}s, Diff=${(timeDiff * 1000).toFixed(1)}ms`)
            
            // Auto-sync if difference is significant
            if (timeDiff > 0.1) {
                console.log('Auto-syncing audio-video timing')
                syncAudioVideo()
            }
        }
    }
...
function startLatencyMonitoring() {
    setInterval(monitorLatency, 100) // Check every 100ms
}
...
window.onload = () => {
    ...
    // Start latency monitoring after a short delay
    setTimeout(startLatencyMonitoring, 2000)
}
...
window.speak = () => {
    ...
    // Unmute audio and sync with video
    const audioElement = document.getElementById('audio')
    if (audioElement) {
        audioElement.muted = false
        // Sync audio-video timing before speaking
        syncAudioVideo()
    }
    ...
    .then(response => {
        if (response.ok) {
            response.text().then(text => {
                ...
                // Sync audio-video after speech starts
                setTimeout(() => {
                    syncAudioVideo()
                }, 100)
            })
